...
Lastly the value readed from analog read is converted to Lux according to ![R_2 = R_1 \bigg( \frac{V_{cc}}{v_0}-1 \bigg)  \quad \land \quad Lux = 10^{\frac{log_{10}(R_2) - b}{m}}](https://latex.codecogs.com/svg.latex?R_2%20=%20R_1%20\bigg(%20\frac{V_{cc}}{v_0}-1%20\bigg)%20%20\quad%20\land%20\quad%20Lux%20=%2010^{\frac{log_{10}(R_2)%20-%20b}{m}}).

### Final remarks

In order not to overload the interruption, the lmminace is written in the led in every iteration, which might lead to some noise. It was not implement a condition due to the fact that it would be more expensive than a basic instruction.
