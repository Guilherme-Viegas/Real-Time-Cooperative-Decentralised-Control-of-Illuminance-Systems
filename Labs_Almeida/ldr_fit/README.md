...
The [ldr_fit.ino](./ldr_fit.ino) is the main code, which calls the functions present in [ldr_fit.cpp](./ldr_fit.cpp) and its respectively header [(ldr_fit.h)](./ldr_fit.cpp).
...
4. Containing of each file:
  ...
  * [multiples1.txt](./text_files/multiples.txt) - multiple tests for m : ```for(i=0; i<21; i++){ m = -0.6 -0.01*i }```
